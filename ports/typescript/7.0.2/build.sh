...
#
# 7.0.2-3 变更（拆双包）：
#   - 二进制不再内嵌主包：主包回到纯 wrapper（tgz 9.2MB→约 3MB），
#     非 OHOS 安装不再拖 22MB 的 arm64 ELF 死重
#   - 新增槽位包 @ohos-npm-ports/typescript-openharmony-arm64，
#     布局镜像上游平台包 @typescript/typescript-linux-arm64
#     （lib/tsc + lib/*.d.ts，os/cpu 限定，preferUnplugged，无 main/bin）
#   - patchs/0002 重写：openharmony 分支从「提前返回内嵌二进制」改为
#     「平台包名指向槽位包」，全平台走同一条平台包解析路径
#   - 上游平台包里的 lib/tsc.sig 是微软发布流水线的产物，运行时不校验
#     （7.0.2-2 未带此文件已在真机跑通），槽位包不带
# ============================================================
...
# ===== build =====
do_build() {
    echo "=== build: 编译 tsgo（容器 aarch64 linux 原生构建，产物同跑 OHOS 真机）==="
